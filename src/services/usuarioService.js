...
async function createUser(data) {
  const { nome, login, senha, perfil } = data;
  ...
  if (!["ADMIN", "OPERADOR"].includes(perfil)) {
    throw new AppError("perfil deve ser ADMIN ou OPERADOR", 400);
  }
...
async function updateUser(id, data) {
  const user = await usuarioRepository.findByIdWithPassword(id);
  ...
  const loginExists = await usuarioRepository.findByLoginExceptId(login, id);
...
async function listUsersForSelect() {
  return usuarioRepository.findAllForSelect();
}
